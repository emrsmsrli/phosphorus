...
impl HitRecord {
    pub fn new(ray: &Ray, location: Point3<f64>, normal: Vector3<f64>, t: f64) -> Self {
        let front_face = ray.direction.dot(&normal) < 0.0;
        HitRecord { location, normal: if front_face { normal } else { -normal }, t, front_face }
    }
}
                ...
                if discriminant > 0.0 {
                    ...
                    for disc in [-disc_sqrt, disc_sqrt].iter() {
                        ...
                        if t_min < root && root < t_max {
                            let pos = ray.at(root);
                            return Some(HitRecord::new(&ray, pos, (pos - center) / *radius, root));
                        }
                    }
                }
